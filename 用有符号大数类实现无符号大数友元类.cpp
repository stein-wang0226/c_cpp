...
class CBigNumber{
    ...
    public:
        CBigNumber();
        //从字符串构造一个无符号大数
        explicit  CBigNumber  (const  string  &str);
        explicit  operator  string  ()  const;  //转换成字符串
        //两个无符号大数相加
        ...
        CBigNumber  operator  *    (const  CBigNumber  &rhs)  const;
        //两个无符号大数比较，小于、等于、大于时分别返回-1、0、1
        int  Compare(const  CBigNumber  &rhs)  const;
        friend    istream  &  operator>>  (istream  &is,  CBigNumber  &rhs);  //输入一个无符号大数
        friend    ostream  &  operator<<  (ostream  &os,  const  CBigNumber  &rhs);  //输出无符号大数
...
CBigNumber:: CBigNumber(){
    m_Unum._lst.push_back(0);
    m_sign=1;
}

CBigNumber::CBigNumber (const  string  &str){
    if(str[0]!='-')m_Unum=CUBigNumber(str),m_sign=1;
    else {
        m_sign=-1;m_Unum=CUBigNumber(str.substr(1,str.size()-1));
    }
}



CBigNumber::operator  string  ()  const{  //转换成字符串
     string  str;
    if(m_sign==-1)str="-";
    for  (auto  it  =  m_Unum._lst.begin();  it  !=  m_Unum._lst.end();  ++it)
                str.push_back(*it  +  '0');//从高位到低位，添加每一位数字，需变成字符
    return str;
}

istream  &  operator  >>  (istream  &is,  CBigNumber  &rhs){
    string str;
    is>>str;
    rhs  =  std::move  (CBigNumber  (str));  //构造的无符号大数转移赋值给rhs
    return is;
}
...
void  CBigNumber::_Normalize(){
    m_Unum._Normalize();
}   


int  CBigNumber::Compare(const  CBigNumber  &rhs)  const{
    if(m_Unum.Compare(rhs.m_Unum)==0)return 0;
    else if(m_sign==1&&rhs.m_sign==-1)return 1;
    else if(m_sign==-1&&rhs.m_sign==1)return -1;
    else if(m_sign==1&&rhs.m_sign==1){
        return m_Unum.Compare(rhs.m_Unum);
    }
    else {
        return -m_Unum.Compare(rhs.m_Unum);
    }
...
CBigNumber  CBigNumber::operator  +  (const  CBigNumber  &rhs)  const{
    CBigNumber result;
    if(this->m_sign==1&&rhs.m_sign==1){
        result.m_sign=1;
        result.m_Unum=this->m_Unum+rhs.m_Unum;
    }
    else if(this->m_sign==-1&&rhs.m_sign==-1){
        result.m_sign=-1;
        result.m_Unum=this->m_Unum+rhs.m_Unum;
    }
    else {//异号
        if(this->m_Unum.Compare(rhs.m_Unum)==1){
            result.m_Unum=this->m_Unum-rhs.m_Unum;
            if(this->m_sign==1) result.m_sign=1;
            else result.m_sign=-1;
        }
        
        else{
            result.m_Unum=rhs.m_Unum-this->m_Unum;
            if(this->m_sign==1) result.m_sign=-1;
            else result.m_sign=1;
        }
    }
...
CBigNumber  CBigNumber::operator  -  (const  CBigNumber  &rhs)  const{
    ...
    if(this->m_sign==1&&rhs.m_sign==-1){
      ...
      result.m_Unum=this->m_Unum+rhs.m_Unum;
          ...
          if(this->m_Unum.Compare(rhs.m_Unum)==1){
            result.m_Unum=this->m_Unum-rhs.m_Unum;
            if(this->m_sign==1) result.m_sign=1;//1 1
            else result.m_sign=-1;   //-1 -1
        }
        else{
            result.m_Unum=rhs.m_Unum-this->m_Unum;
            if(this->m_sign==1) result.m_sign=-1;
...
CBigNumber  CBigNumber::operator  *    (const  CBigNumber  &rhs)  const{
    CBigNumber result;
    result.m_Unum=this->m_Unum*rhs.m_Unum;
    result.m_sign=this->m_sign*rhs.m_sign;
    return result;
}
int  main  ()
{
        CBigNumber  A,  B,  C,  D,  E;
        cin  >>  A  >>  B;  //有符号大数输入
        C  =  A  +  B;  //有符号大数相加
        D  =  A  -  B;  //有符号大数相减
        E  =  A  *  B;  //有符号大数相减
